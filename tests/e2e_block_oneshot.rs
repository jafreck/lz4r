...
use lz4::{
    ...
    lz4_compress_default as compress_default, lz4_decompress_safe,
    decompress_safe_partial,
...
fn test_compress_default_roundtrip_typical_data() {
    ...
    let compressed_size = compress_default(&original, &mut compressed)
...
fn test_compress_bound_returns_adequate_size() {
    ...
    for &size in &input_sizes {
        ...
        assert!(bound > 0, "compress_bound should return positive value for size {}", size);
        
        let mut dst = vec![0u8; bound as usize];
        let result = compress_default(&input, &mut dst);
        
        assert!(result.is_ok(), "compression with compress_bound buffer should succeed for size {}", size);
        let compressed_size = result.unwrap();
        assert!(compressed_size <= bound as usize, 
...
fn test_compress_fast_max_acceleration_roundtrip() {
    ...
    assert!(compressed_size < original.len() / 10, 
...
fn test_compress_dest_size_fills_destination_exactly() {
    ...
    if src_consumed > 0 && compressed_size > 0 {
        ...
        assert_eq!(&decompressed[..decompressed_size], &original[..src_consumed]);
    }
}
...
fn test_empty_input_roundtrip() {
    ...
    assert!(compressed_size <= 16, "empty input should produce small output");
    ...
    assert_eq!(decompressed_size, 0, "decompressed empty input should yield 0 bytes");
}
...
fn test_decompress_safe_partial_stops_early() {
    ...
    assert!(decompressed_size <= target_output_size,
