...
fn small_file_produces_valid_lz4_frame() {
    ...
    assert_eq!(in_size, data.len() as u64, "in_stream_size must match input length");
    let compressed = std::fs::read(&dst).unwrap();
    assert!(!compressed.is_empty(), "output must be non-empty");
    assert!(starts_with_lz4_magic(&compressed), "output must start with LZ4 magic");
}
...
fn small_file_compressed_is_smaller_than_input() {
    ...
    let data: Vec<u8> = b"AAAAAAAAAAAAAAAAAAAAAAAAA".iter().cycle().take(256 * 1024).copied().collect();
    std::fs::write(&src, &data).unwrap();
    ...
    compress_filename_mt(&mut in_size, &mut ress, src.to_str().unwrap(), dst.to_str().unwrap(), 1, &prefs)
        .expect("compress");
...
fn multi_block_file_produces_non_empty_output() {
    ...
    assert!(starts_with_lz4_magic(&c_data), "output must start with LZ4 magic");
}
...
fn multi_block_in_stream_size_matches_input_len() {
    ...
    assert_eq!(in_size, data.len() as u64, "in_stream_size mismatch for 9 MB file");
}
...
fn compression_level_9_produces_valid_output() {
    ...
    let mut ress = make_ress(&prefs);
    let mut in_size = 0u64;
    compress_filename_mt(&mut in_size, &mut ress, src.to_str().unwrap(), dst.to_str().unwrap(), 9, &prefs)
        .unwrap();
    let out = std::fs::read(&dst).unwrap();
    assert!(starts_with_lz4_magic(&out));
}
...
fn remove_src_file_true_deletes_source() {
    ...
    assert!(!src.exists(), "source file must be deleted when remove_src_file is true");
...
fn remove_src_file_false_keeps_source() {
    ...
    assert!(src.exists(), "source file must be kept when remove_src_file is false");
}
...
fn multi_block_output_ends_with_end_mark_no_checksum() {
    ...
    assert!(c.len() >= 4);
    let end = &c[c.len() - 4..];
    assert_eq!(end, &[0u8; 4], "last 4 bytes must be end-of-data mark 0x00000000 when checksum disabled");
}
...
fn multi_block_output_with_checksum_has_end_mark_plus_4_bytes() {
    ...
    assert!(c.len() >= 8, "output must have at least 8 trailing bytes for end-mark + checksum");
