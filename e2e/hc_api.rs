...
use lz4::hc::{
    compress_hc, compress_hc_continue, reset_stream_hc, Lz4StreamHc, LZ4HC_CLEVEL_DEFAULT,
    LZ4HC_CLEVEL_MAX, LZ4HC_CLEVEL_MIN,
};
use lz4::{compress_bound, lz4_compress_default, lz4_decompress_safe};
...
fn test_hc_default_level_roundtrip() {
    ...
    assert!(
        ...
        "HC should compress repetitive data"
    );

    // Decompress using standard block API
    let mut decompressed = vec![0u8; original.len()];
    let result = lz4_decompress_safe(&compressed[..compressed_size as usize], &mut decompressed);
...
fn test_hc_vs_fast_compression_ratio() {
    ...
    let fast_size =
        lz4_compress_default(&original, &mut compressed_fast).expect("Fast compression failed");
    ...
    let result_hc = lz4_decompress_safe(&compressed_hc[..hc_size as usize], &mut decompressed);
    assert!(result_hc.is_ok(), "HC decompression failed");
    assert_eq!(decompressed, original, "HC roundtrip mismatch");

    let result_fast = lz4_decompress_safe(&compressed_fast[..fast_size], &mut decompressed);
...
fn test_hc_streaming_reset() {
    ...
    assert!(
        result1.is_ok(),
        "First message decompression failed: {:?}",
        result1
    );
    ...
    assert!(
        result2.is_ok(),
        "Second message decompression failed: {:?}",
        result2
    );
