...
use lz4::{
    compress_fast, decompress_safe_partial, lz4_compress_default as compress_default,
    lz4_decompress_safe as decompress_safe, lz4f_compress_frame, lz4f_decompress, DecompressError,
    Lz4Error, LZ4_MAX_INPUT_SIZE,
};
...
fn test_decompress_dst_too_small() {
    ...
    let comp_size = compress_default(src, &mut compressed).expect("compression should succeed");
    ...
    assert!(
        result.is_err(),
        "decompress_safe should return Err when dst is too small"
    );
...
fn test_decompress_corrupt_data() {
    ...
    assert!(
        result.is_err(),
        "decompress_safe should return Err on corrupt data"
    );
...
fn test_decompress_empty_input() {
    ...
    assert!(
        result.is_err(),
        "decompress_safe should return Err on empty input"
    );
}
...
fn test_compress_dst_empty() {
    ...
    assert!(
        result.is_err(),
        "compress_default should return Err when dst is empty"
    );
...
fn test_compress_dst_too_small() {
    ...
    assert!(
        result.is_err(),
        "compress_default should return Err when dst is too small"
    );
...
fn test_frame_decompress_truncated() {
    ...
    let frame_size =
        lz4f_compress_frame(&mut frame_buf, src, None).expect("frame compression should succeed");
    ...
    let result = lz4f_decompress(&mut dctx, Some(&mut dst), truncated, None);
...
fn test_frame_decompress_invalid_magic() {
    ...
    let result = lz4f_decompress(&mut dctx, Some(&mut dst), invalid_frame, None);

    // Should return an error for invalid magic
    assert!(
        result.is_err(),
        "lz4f_decompress should return Err for invalid magic"
    );
...
fn test_frame_decompress_all_zeros() {
    ...
    let result = lz4f_decompress(&mut dctx, Some(&mut dst), &zeros, None);

    // All zeros is not a valid frame magic, should error
    assert!(
        result.is_err(),
        "lz4f_decompress should return Err for all-zero data"
    );
}
...
fn test_compress_input_too_large() {
    ...
    assert_eq!(
        bound, 0,
        "compress_bound should return 0 for input > LZ4_MAX_INPUT_SIZE"
    );
    ...
    assert!(
        bound_at_limit > 0,
        "compress_bound should accept input at exactly LZ4_MAX_INPUT_SIZE"
    );
}
...
fn test_decompress_partial_target_exceeds_dst() {
    ...
    match result {
        Ok(size) => {
            assert!(
                size <= dst.len(),
                "returned size should not exceed dst.len()"
            );
        }
        Err(DecompressError::MalformedInput) => {
...
fn test_roundtrip_single_byte() {
    ...
    let comp_size =
        compress_default(src, &mut compressed).expect("compressing single byte should succeed");
...
fn test_frame_decompress_partial_header() {
    ...
    let result = lz4f_decompress(&mut dctx, Some(&mut dst), partial_header, None);
