...
use lz4::frame::{
    header::lz4f_compress_frame_bound, lz4f_compress_frame, lz4f_decompress, ContentChecksum,
    Lz4FDCtx, Preferences,
};
...
fn test_compress_frame_roundtrip_default_prefs() {
    ...
    assert!(
        compressed_size <= frame_bound,
        "compressed size should not exceed bound"
    );
...
fn test_compress_frame_magic_bytes() {
    ...
    assert!(
        compressed_size >= 4,
        "compressed frame must be at least 4 bytes for magic"
    );
...
fn test_compress_frame_roundtrip_large_input() {
    ...
    let original: Vec<u8> = pattern.iter().cycle().take(1_500_000).copied().collect();
...
fn test_compress_frame_empty_input() {
    ...
    assert_eq!(
        ...
        "decompressed empty frame should be empty"
    );
}
